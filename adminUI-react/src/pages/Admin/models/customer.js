import { fetchlist, deleteObject, addObject, updateObject } from '@/services/customer';
...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put, select }) {
      let { pageSize, current } = yield select(state => state.customer.data.pagination);
    ...
    *reload(action, { put, select }) {
      let { pageSize, current, total } = yield select(state => state.customer.data.pagination);
