...
public class DictController extends BaseController {
    ...
    public R<SysDict> dict(@PathVariable Integer id) {
        return new R<>(Boolean.TRUE, sysDictService.selectById(id));
    }
     ...
     * @return 返回所有字典
     */
    @GetMapping("/allType/{locale}")
    public R<Map<String, List<SysDict>>> findDictByAllType(@PathVariable String locale) {
        SysDict condition = new SysDict();
        condition.setDelFlag(CommonConstant.STATUS_NORMAL);
        List<SysDict> sysDictList = sysDictService.selectList(new EntityWrapper<>(condition));

        log.info("findDictByAllType(): locale {}",locale);
        if (CommonConstant.ENUS.equals(locale)) {
            for (SysDict sysDict : sysDictList) {
                String labelEnUs = sysDict.getLabelEnUs();
                sysDict.setLabel(labelEnUs);
            }
        }

        Map<String, List<SysDict>> sysDictListMap = sysDictList.stream().collect(Collectors.groupingBy(SysDict::getType));
        return new R<>(Boolean.TRUE, sysDictListMap);
    }
