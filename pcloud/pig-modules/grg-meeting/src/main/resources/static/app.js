...
function logIn() {
	...
	httpRequest('POST', 'sessions/login', jsonBody, 'Login WRONG', function successCallback(response) {
...
function logOut() {
	httpRequest('GET', 'sessions/logout', null, 'Logout WRONG', function successCallback(response) {
...
function getSessionIdAndToken(callback) {
	...
	// Send POST request
	httpRequest('POST', 'sessions/get-sessionid-token', jsonBody, 'Request of SESSIONID and TOKEN gone WRONG:', function successCallback(response) {
		sessionId = response[0]; // Get sessionId from response
		token = response[1]; // Get token from response
		console.info('Request of SESSIONID and TOKEN gone WELL (SESSIONID:' + sessionId + ", TOKEN:" + token + ")");
...
function removeUser() {
	...
	// Send POST request
	httpRequest('POST', 'sessions/remove-user', jsonBody, 'User couldn\'t be removed from session', function successCallback(response) {
...
function httpRequest(method, url, body, errorMsg, callback) {
	...
	var serverUrl = 'http://10.1.39.108:4040/' + url;
	http.open(method, serverUrl, true);
