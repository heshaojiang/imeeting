...
public class TokenArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public Object resolveArgument(MethodParameter methodParameter,
        ...
        return optional.orElseGet(() -> generatorByToken(token));
    }

    private UserVo generatorByToken(String token) {
        UserVo userVo = UserUtils.getUserVoFromToken(token);
        cacheManager.getCache(SecurityConstants.TOKEN_USER_DETAIL).put(token, userVo);
        System.out.print("token是否放入："+ cacheManager.getCache(SecurityConstants.TOKEN_USER_DETAIL).get(token, UserVo.class));
        return userVo;
    }
